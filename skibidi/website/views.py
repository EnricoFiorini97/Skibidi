...
def index(request):
    ...
    return render(request, 'index-2.html', {'kind_list':k_list, 'page_obj':page_obj})
...
def anime_ep(request, anime, stagione, ep):
    ...
    return render(request, 'media.html', {'query':querysetAnime[0], 'kind_list':k_list,'anime': anime,'stagione':stagione,'ep':ep, 'ep_link':serialized_Episode.data["path"]})

def anime_ep_list(request, anime, stagione):
    querysetAnime = Anime.objects.filter(name=anime, season=stagione)
    serialized_Anime = AnimeSerializer(querysetAnime[0])
    return render(request, 'index_episodes.html',{'anime':anime, 'season':stagione,'kind_list':k_list, 'ep_list':range(serialized_Anime.data["start_number_episode"], serialized_Anime.data["last_episode"]+1)})
